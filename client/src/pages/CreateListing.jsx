...
export default function CreateListing() {
    ...
    const handleSubmit= async(e)=>{
        e.preventDefault();
        try {
            if (formData.imageUrls.length < 1) return setError('Must upload at least one image!')
            if (+formData.regularPrice < +formData.discountPrice) return setError('Discounted price must be lower!')
            setLoading(true)
            setError(false)
    ...
    <main className='p-3 max-w-4xl mx-auto'>
        ...
        <div className='flex gap-7 flex-wrap'>
          ...
          checked={formData.type==='rent'}/>
            ...
            <div className='flex gap-2'>
                <input type="checkbox" id='parkingSpot' className='w-5' onChange={handleChange}
        ...
        <div className='flex flex-wrap gap-7'>
            <div className='flex items-center gap-2'>
                <input type="number" id='bedrooms' min='1' max='15' required onChange={handleChange}
          value={formData.bedrooms} className='p-3 border border-gray-300 rounded-lg'/>
                <p className='font-semibold'>Beds</p>
            </div>
            <div className='flex items-center gap-2'>
                <input type="number" id='bathrooms' min='1' max='8' required onChange={handleChange}
          value={formData.bathrooms} className='p-3 border border-gray-300 rounded-lg'/>
                <p className='font-semibold'>Baths</p>
            </div>
            <div className='flex items-center gap-2'>
                <input type="number" id='regularPrice' min='50' max='1000000' required onChange={handleChange}
          value={formData.regularPrice} className='p-3 border border-gray-300 rounded-lg'/>
            ...
            {formData.offer && (
            <div className='flex items-center gap-2'>
                <input type="number" id='discountPrice' min='0' max='1000000' required onChange={handleChange}
          value={formData.discountPrice} className='p-3 border border-gray-300 rounded-lg'/>
                <div className='flex flex-col items-center'>
        ...
        <button disabled={loading || uploading} className='p-3 bg-gray-800 text-white text-center rounded-lg uppercase hover:opacity-80 disabled:opacity-70'>
